...
-------------------------------------------------------
local debug = false;
local echo = false; -- Print in nas GUI
                    ...
                    do
                        ...
                        if #result > 0 and rc == 0 then
                            ...
                            if(hubResult.domain ~= hub.domain or hubResult.ip ~= hub.ip or hubResult.origin ~= hub.origin or hubResult.version ~= hub.version) then
                                local _, u_rc = database.query("UPDATE hubs_list SET domain='"..hub.domain.."',ip='"..hub.ip.."',origin='"..hub.origin.."',versions='"..hub.version.."' WHERE name LIKE '"..hub.name.."'");
                                if u_rc ~= 0 then
                                    log("       HUB "..hub.name.." : Update failed",true);
                                end
                            end
                        else
                            local insert = "INSERT INTO hubs_list(domain,name,ip,origin,versions) ";
                            local values = "VALUES ('"..hub.domain.."','"..hub.name.."','"..hub.ip.."','"..hub.origin.."','"..hub.version.."')";
                    ...
                    if hub_find then
                        ...
                        if robotlist ~= nil then
                            ...
                            for _,robot in search(robotlist) do
                                ...
                                -------------------------------------------------------
                                log('   Requested ROBOT__NAME : '..robot.name,true);
                                        ...
                                        do
                                            ...
                                            if #result > 0 and rc == 0 then
                                                ...
                                                if(robotResult.ip ~= robot.ip or robotResult.origin ~= robot.origin or robotResult.version ~= robot.version) then
                                                    local _, u_rc = database.query("UPDATE robots_list SET ip='"..robot.ip.."',origin='"..robot.origin.."',versions='"..robot.version.."' WHERE name LIKE '"..robot.name.."'");
                                                    if u_rc ~= 0 then
                                                        log("       => Robot update failed on robot : "..robot.name,true);
                                                    end
                                                end
                                            else
                                                local insert = "INSERT INTO robots_list(hubid,name,ip,origin,versions) ";
                                                local values = "VALUES ('"..HUB_FK.."','"..robot.name.."','"..robot.ip.."','"..robot.origin.."','"..robot.version.."')";
                                                local _, i_rc = database.query(insert..values);
                                                if i_rc ~= 0 then
                                                        ...
                                                        do
                                                            ...
                                                            if #result > 0 and rc == 0 then
                                                                if(result[1].active ~= probe.active or result[1].version ~= probe.pkg_version) then
                                                                    local _,u_rc = database.query("UPDATE probes_list SET active='"..probe.active.."',versions='"..temp_version.."' WHERE name LIKE '"..probeName.."'");
                                                                    if u_rc ~= 0 then
                                                                        log("       Fail update from probe => "..probeName,true);
                                                                    end
                                                                end
                                                            else
                                                                local insert = "INSERT INTO probes_list(robotid,name,active,versions) ";
                                                                local values = "VALUES ('"..ROBOT_FK.."','"..probeName.."','"..probe.active.."','"..temp_version.."')";
                                                                local _, i_rc = database.query(insert..values);
                                                                if i_rc ~= 0 then
...
if #R > 0 and rc == 0 then

    for _,robotinfo in search(R) do

        local ADDR = "/"..robotinfo.domain.."/"..robotinfo.hubname.."/"..robotinfo.robotname.."/controller";
        local probes_list, pl_rc = database.query("SELECT name,id FROM probes_list WHERE robotid='"..robotinfo.robotid.."'");
        ...
        if #probes_list > 0 and pl_rc == 0 then
            ...
            for _,probe in search(probes_list) do
                ...
                if probe_configuration ~= nil then
                    ...
                    probeConfig, probe_rc = nimbus.request(ADDR,'probe_config_get', args);
                                        ...
                                        do
                                            ...
                                            if selectROBOT ~= nil and robotRC == 0 then
                                                if selectROBOT[1].svalue ~= v then
                                                    ...
                                                    if u_rc ~= 0 then
                                                        log("   "..robotinfo.robotname.." : Probe configuration update failed, probe name : "..probe.name,true);
                                                    end
                                                end
                                            else
                                                local _,i_rc = database.query("INSERT INTO probes_config(probeid,section,skey,svalue) VALUES ('"..probe.id.."','"..S_msg.."','"..k.."','"..v.."')")
                                                if i_rc ~= 0 then
                                                    log("   "..robotinfo.robotname.." : Probe configuration insert failed, probe name : "..probe.name.." and rc code = "..tostring(i_rc),true);
                                                end
                                            end
            ...
            log("   => No probes find on robot =>  "..robotinfo.robotname,true);
        end
